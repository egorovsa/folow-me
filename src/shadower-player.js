...
export class FollowPlayer {
    ...
    playBlocks() {
        ...
        for (let i = 0; i < this.recordedObjects.length - 1; i++) {
            ...
            if (this.recordedObjects[i].objectId && this.recordedObjects[i].type === 'keyup') {
                textObject[this.recordedObjects[i].objectId] = this.recordedObjects[i].value;
            }

            if (this.recordedObjects[i].objectId && this.recordedObjects[i].type === 'scroll') {
                scrollObject[this.recordedObjects[i].objectId] = this.recordedObjects[i].elementScrollTop;
            }

            if (this.recordedObjects[i].type === 'focus' || this.recordedObjects[i].type === 'blur') {
                let name = 'window';
                let tag = 'window';
        ...
        if (paste.timeSpent) {
            this.showNotification(
                'Вставлен текст в: ' + paste.tag + ' (' + paste.pastedData + ') ', true
            );
        }

        for (let obj in focusObjects) {
            if (focusObjects.hasOwnProperty(obj)) {
                if (focusObjects[obj].type === 'focus' && this.currentTimeMs - focusObjects[obj].timeSpent < 200) {
                    ...
                    // unlock if you need to show this notification
        ...
        for (let obj in textObject) {
            if (textObject.hasOwnProperty(obj)) {
        ...
        for (let obj in scrollObject) {
            if (scrollObject.hasOwnProperty(obj)) {
                let field = document.getElementById(obj);

                if (field.scrollTop !== scrollObject[obj]) {
                    field.scrollTop = scrollObject[obj];
                }
            }
        }
    }
